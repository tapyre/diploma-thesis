...
\begin{itemize}
  \item ReAct-Loop: \texttt{AgentType.ZERO\_SHOT\_REACT\_DESCRIPTION} sorgt dafür, dass das LLM selbst entscheidet, wann ein Tool benutzt werden soll. Es erzeugt intern eine Folge aus \enquote{Thought}, \enquote{Action} und \enquote{Observation}.
  \item Tool-Auswahl: Die Liste \texttt{tools} wird später aus den geladenen Plugins erzeugt. Das LLM sieht nur die Tool-Beschreibungen und entscheidet basierend darauf, welches Tool geeignet ist.
  \item max\_iterations: Begrenzung des ReAct-Loops auf zwei Tool-Aufrufe, um Endlosschleifen zu vermeiden \cite{schick2023toolformer}.
\end{itemize}
...
\begin{itemize}
  \item Interface-basiert: Plugins müssen nur \texttt{run()} implementieren.
  \item Loose Coupling: Der Agent kennt nur die LangChain-Tools, nicht die konkrete Plugin-Klasse.
  \item Tool-Integration: \texttt{LCTool.from\_function} macht aus \texttt{run()} ein Tool, das im ReAct-Loop genutzt werden kann \cite{langchain2023}.
\end{itemize}
...
\begin{itemize}
  \item Plugin-basiertes Design: Funktionen werden modular ergänzt.
  \item Keine Codeänderung im Kern: Neue Funktionalitäten sind sofort nutzbar.
  \item Reflection \& Introspection: Automatische Erkennung von Plugin-Klassen.
\end{itemize}
