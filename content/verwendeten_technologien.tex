...
\begin{itemize}
    \item Namespaces: isolieren Prozesse, Netzwerke, Benutzer und Dateisysteme.
    \item Control Groups (cgroups): begrenzen CPU-, RAM- und I/O-Ressourcen.
    \item Union-Filesystems (z.\,B. OverlayFS): ermöglichen effiziente Layer-basierte Images.
\end{itemize}
...
\begin{itemize}
    \item Clustered Index: InnoDB speichert Daten entlang des Primärschlüssels. Die Tabelle ist selbst ein B{+}-Baum.
    \item Secondary Indexes: weitere B{+}-Bäume, die Zeiger auf die Primärzeilen enthalten.
    \item Effiziente Suche: Durch die logarithmische Höhe der B{+}-Bäume können Suchanfragen schnell ausgeführt werden.
\end{itemize}
...
\begin{itemize}
    \item Atomicity: Eine Transaktion wird entweder vollständig ausgeführt oder verworfen.
    \item Consistency: Alle Daten erfüllen definierte Integritätsregeln.
    \item Isolation: Gleichzeitige Transaktionen beeinflussen sich nicht.
    \item Durability: Bestätigte Änderungen bleiben dauerhaft gespeichert.
\end{itemize}
...
\begin{itemize}
    \item Vektoren (meist 768 oder 1024 Dimensionen),
    \item Payload-Daten wie Titel, DOI, Autoren oder Jahr,
    \item Segmenten zur internen Aufteilung der Daten.
\end{itemize}
...
\begin{itemize}
    \item GET: Anfrage von Daten
    \item POST: Erstellen neuer Daten
    \item PUT/PATCH: Aktualisieren von Daten
